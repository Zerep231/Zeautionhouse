...
public class DeliveryRepository {
    ...
    public synchronized int insert(UUID playerUuid, Delivery.Type type, ItemStack item, int amount, String currency) {
    ...
    public synchronized boolean claim(int id) {
        try (PreparedStatement ps = db.getConnection().prepareStatement("DELETE FROM deliveries WHERE id=?")) {
            ps.setInt(1, id);
