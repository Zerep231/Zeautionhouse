...
public class ShopManager {
    ...
    public void purchase(Player player, ShopItem item, int amount) {
        ...
        if (!plugin.getCurrencyManager().has(player, total, item.getCurrencyId())) {
            player.sendMessage(plugin.getLang().format("shop.not-enough-items",
                "price", plugin.getCurrencyManager().format(total, item.getCurrencyId())));
            return;
        }
        ...
        plugin.getCurrencyManager().withdraw(player, total, item.getCurrencyId());
