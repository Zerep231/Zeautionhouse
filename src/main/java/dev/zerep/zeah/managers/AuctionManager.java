...
public class AuctionManager {
    ...
    public void completeSell(Player player, CreateSession session) {
        ...
        plugin.getDb().insertListing(player.getUniqueId(), player.getName(), data, price, session.getCurrencyId(), expireAt)
            .thenAccept(id -> {
                ...
                plugin.getAuditLogger().log("SELL", player.getUniqueId(),
                    player.getName() + " listed " + item.getType() + " for " + price + "x " + session.getCurrencyId());
            }).exceptionally(ex -> {
    ...
    public void purchase(Player player, int listingId) {
        plugin.getDb().getListingById(listingId).thenAccept(listing -> {
            Bukkit.getScheduler().runTask(plugin, () -> {
                ...
                ItemStack[] currencyStacks = plugin.getCurrencyManager().createCurrencyStacks(price, listing.getCurrencyId());
                ...
                plugin.getDb().executePurchase(
                    ...
                    if (!success) {
                        ...
                        Bukkit.getScheduler().runTask(plugin, () -> {
                            plugin.getCurrencyManager().deposit(player, price, listing.getCurrencyId());
                    ...
                    plugin.getAuditLogger().log("BUY", player.getUniqueId(),
                        player.getName() + " bought listing #" + listingId + " for " + price + "x " + listing.getCurrencyId());

                }).exceptionally(ex -> {
                    plugin.getLogger().severe("Purchase failed: " + ex.getMessage());
                    listing.setStatus(Listing.Status.ACTIVE);
                    Bukkit.getScheduler().runTask(plugin, () -> plugin.getCurrencyManager().deposit(player, price, listing.getCurrencyId()));
                    return null;
                });
            });
        });
