...
public class ShopConfirmGUI extends AuctionGUI {
    ...
    private void render() {
        ...
        boolean canAfford = plugin.getCurrencyManager().has(player, totalPrice, shopItem.getCurrencyId());

        inventory.setItem(13, buildItem(shopItem.getMaterial(),
            "&e" + quantity + "x " + ColorUtil.formatMaterial(shopItem.getMaterial().name()),
            List.of(
                "&7Unit price:    &6" + plugin.getCurrencyManager().format(shopItem.getPrice(), shopItem.getCurrencyId()),
                "&7Total:         &a" + plugin.getCurrencyManager().format(totalPrice, shopItem.getCurrencyId()),
                "&7Your balance:  " + (canAfford ? "&e" : "&c") + plugin.getCurrencyManager().format(plugin.getCurrencyManager().getBalance(player, shopItem.getCurrencyId()), shopItem.getCurrencyId()),
                "",
                canAfford ? "&aYou can afford this!" : "&cNot enough " + plugin.getCurrencyManager().getCurrency(shopItem.getCurrencyId()).name() + "!"
            )));
        ...
        inventory.setItem(11, buildItem(canAfford ? Material.GREEN_CONCRETE : Material.GRAY_CONCRETE,
            ...
            List.of("&7Pay " + plugin.getCurrencyManager().format(totalPrice, shopItem.getCurrencyId()))));
    ...
    public void handleClick(int slot, ClickType click, InventoryClickEvent event) {
        ...
        switch (slot) {
            case 11 -> {
                if (!plugin.getCurrencyManager().has(player, totalPrice, shopItem.getCurrencyId())) {
                    player.sendMessage(plugin.getLang().format("shop.not-enough-items",
                        "price", plugin.getCurrencyManager().format(totalPrice, shopItem.getCurrencyId())));
                    render();
                    return;
                }
